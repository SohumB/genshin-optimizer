  ...
  BuildGeneratorEditorCard = ({ statsDisplayKeys, initialStats }) => {
    ...
    if (generatingBuilds) {
      ...
      buildAlert = <Alert variant="success">
        <span>Generating and testing <b><tt>{generationProgressString}/{totalUnskippedString}</tt></b> build configurations against the criteria for <b>{characterName}</b> (<b>{generationSkippedString}</b> skipped)</span><br />
        <h6>Time elapsed: <tt>{timeStringMs(generationDuration)}</tt></h6>
        <ProgressBar now={progPercent} label={`${progPercent.toFixed(1)}%`} />
      </Alert>
    } else if (!generatingBuilds && generationProgress) {//done
      buildAlert = <Alert variant="success">
        <span>Generated and tested <b>{totalUnskippedString}</b> Build configurations against the criteria for <b>{characterName}</b> (<b>{generationSkippedString}</b> skipped)</span>
        <h6>Total duration: {timeStringMs(generationDuration)}</h6>
